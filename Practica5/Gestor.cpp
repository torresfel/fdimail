...
bool arrancar(tGestor &gestor, string dominio){
	...
	if (cargarUsuarios(gestor.usuarios, dominio)){
		...
		if (cargarLCorreos(gestor.listaCorreos, dominio)){
			...
			inicializarLCorreos(gestor.listaCorreos, MAX_CORREOS);
		}
	}
	else{
...
void apagar(tGestor &gestor){
	...
	destruirLCorreos(gestor.listaCorreos);
	destruirUsuarios(gestor.usuarios);
}
...
bool crearCuenta(tGestor &gestor){
	...
	usuarioAux.nombre = usuarioAux.nombre + "@" + gestor.dominio;
	cout << "Contrasenia: ";
	...
	if (!buscarUsuario(gestor.usuarios, usuarioAux.nombre, pos)){
		if (aniadir(gestor.usuarios, usuarioAux)){
			...
			inicializarLReg(gestor.usuarios.elementos[pos]->entrada, MAX_REG);
			inicializarLReg(gestor.usuarios.elementos[pos]->salida, MAX_REG);
			ok = true;
...
void enviarCorreo(tGestor &gestor, const tCorreo &correo){
	...
	if (insertarLCorreos(gestor.listaCorreos, correo)){
		...
		if (insertar(gestor.usuarios.elementos[gestor.numUsuarios]->salida, registro)){
			registro.leido = false;
			if (buscarUsuario(gestor.usuarios, correo.destinatario, pos)){
				insertar(gestor.usuarios.elementos[pos]->entrada, registro);
			}
			else{
			...
			cout << "Error al enviar el email. Vuelva a intentarlo" << endl;
			system("pause");
		}
	}
	else{
...
void lecturaRapida(tGestor &gestor, tListaRegistros &listaReg){
	tCorreo correo;
	tListaCorreos listaCorreosAux;
	int pos;
	inicializarLCorreos(listaCorreosAux, listaReg.cont); //creamos una lista de correos que se modifica solo para los correos del usuario logeado, con capacidad igual a todos sus emnsajes recibidos sin leer
	system("cls");
	for (int i = 0; i < listaReg.cont; i++){
		if (listaReg.elementosReg[i].leido == false){
			...
			listaCorreosAux.elementosCorreo[listaCorreosAux.cont] = correo;
			listaCorreosAux.cont++;
		}
	}
	ordenar_AF(listaCorreosAux);
	for (int i = 0; i < listaCorreosAux.cont; i++){
		cout << aCadena(listaCorreosAux.elementosCorreo[i]);
	}
	system("pause");
	destruirLCorreos(listaCorreosAux);
}
